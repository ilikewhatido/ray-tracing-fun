...
// Operators
		Vector3d& operator = (const Vector3d& A)          // ASSIGNMENT (=)
			{ x=A.x; y=A.y; z=A.z; m_bSelected = A.m_bSelected;
				return(*this);  };
...
//
	...
	double getAngleDeg(const Vector3d<float> &other) 
	{
		if((isZero())||(other.isZero()))
			return 0.0f;

		double deg_angle = RADTODEG(getAngleRad(other));

		//if(deg_angle > 180.0f)
		//	deg_angle = 360.0f - deg_angle;

		return deg_angle;
	}

	double getAngleRad(const Vector3d<float> &other)
	{
		if((isZero())||(other.isZero()))
			return 0.0f;
		...
		rad_angle = float(acos(Dot(other)));
...
typedef Vector3d<float> CVector3D;
typedef Vector2d<float> CVector2D;
